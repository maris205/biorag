...
On an in-index sequence-window stress test, vector retrieval recovers substantial but lower biological match than BLAST. Held-out parent controls select ProtT5 for protein partitions and DNABERT-2 for the auxiliary DNA candidate route, while BLAST remains stronger for verification. We then build a 2,000-protein SeqLit-DAG linking sequence candidates to GO/GOA and PubMed evidence. On an accession-masked 33/66 development/test split, a frozen Graph-IDF selector raises mean function and literature F1 over a budget-matched rank-first selector from 0.071 to 0.100 and from 0.085 to 0.100. Qwen2.5-7B-Instruct executes the 66 test packs with GO/PMID evidence-selection F1 of 0.985/1.000, citation entailment of 1.000, zero out-of-pack identifier hallucination, and correct abstention whenever mechanism evidence is absent. These structured tasks do not establish free-form biomedical reasoning or mechanism discovery; they demonstrate a reproducible sequence-to-literature evidence interface for local agents.
\end{abstract}
...
We next tune Graph-IDF only on 33 development queries and freeze it on the remaining 66. The selected configuration uses the first five protein candidates, IDF power 1.0, and three output claims. Table~\ref{tab:agent_selector_eval} compares it with a rank-first selector at the same output budget. Paired query bootstrap intervals for the Graph-IDF F1 gain are $[0.006,0.057]$ for function and $[0.000,0.034]$ for literature.

\begin{table}[t]
\centering
\caption{Frozen selector results on 66 accession-masked test queries. The retrieval oracle measures whether a correct identifier exists in the available pack and is not executable.}
...
\midrule
Rank-first, matched budget & 0.071 & 0.152 & 0.085 & 0.197 & -- & -- \\
Graph-IDF & \textbf{0.100} & \textbf{0.212} & \textbf{0.100} & \textbf{0.227} & +0.029 & +0.015 \\
Retrieval oracle & 0.431 & 0.515 & 0.153 & 0.258 & -- & -- \\
\bottomrule
\end{tabular}}
\end{table}

Finally, Qwen2.5-7B-Instruct executes three structured questions for every frozen test pack: supported GO terms, supported PMIDs, and whether mechanism evidence is sufficient. Table~\ref{tab:generated_agent_eval} separates end-to-end label F1 from evidence selection and citation entailment. All 198 outputs satisfy the two-line format and bracketed citation syntax. Three function outputs choose a later valid GO item instead of the requested third Graph-IDF item; these frozen errors produce selection F1 0.985. No output introduces a GO ID, PMID, or citation outside its pack, and all mechanism prompts correctly abstain.

\begin{table}[t]
\centering
\caption{Generated Agent evaluation on 66 held-out queries. Entailment checks whether every cited identifier is supported by its typed evidence edge. Hallucination is the rate of identifiers outside the supplied pack.}
\label{tab:generated_agent_eval}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrrr}
\toprule
QA type & Answer F1 & Retrievable F1 & Selection F1 & Citation valid & Entailment & Hallucination & Correct abstention \\
\midrule
Function & 0.100 & 0.111 & 0.985 & 1.000 & 1.000 & 0.000 & -- \\
Literature & 0.100 & 0.138 & 1.000 & 1.000 & 1.000 & 0.000 & -- \\
Mechanism & -- & -- & -- & 1.000 & 1.000 & 0.000 & 1.000 \\
\bottomrule
\end{tabular}}
\end{table}

Generation uses BF16 and peaks at 14.35~GiB, so the experiment fits a 32~GB GPU. Model load takes 5.43~s; resident-model generation averages 607~ms per answer at 74.75 generated tokens/s. These values measure constrained evidence execution, not interactive retrieval plus embedding latency. The low answer F1 relative to citation reliability also identifies the next bottleneck: increasing retrieval and compact-selector coverage without admitting a noisy paper pool. Expanding the paper budget from 20 to 200 doubles the test literature-oracle hit from 0.258 to 0.515, but does not improve the current selector, demonstrating that unfiltered graph expansion raises both ceiling and noise.
